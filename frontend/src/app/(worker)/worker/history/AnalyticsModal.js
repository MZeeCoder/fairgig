  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {/* Content Body */}
        <div
          className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50/20"
          id="analytics-pdf-content"
        >
          {!data ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-slate-900">
                    PKR{" "}
                    {data.summary?.total_verified_net?.toLocaleString() || 0}
                  </p>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-slate-900">
                    PKR{" "}
                    {data.summary?.current_hourly_rate?.toLocaleString() || 0}
                    /hr
                  </p>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm md:col-span-2">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-sm font-medium text-slate-500">
                      City Benchmark ({data.benchmarks?.city || "N/A"})
                    </p>
                  </div>
                  <div className="flex items-end justify-between">
                    <p className="text-2xl font-bold text-slate-900">
                      PKR {data.benchmarks?.city_median?.toLocaleString() || 0}
                      /hr
                    </p>
                    <span
                      className={`text-sm font-medium px-2.5 py-1 rounded-full ${
                        (data.benchmarks?.status || "").includes("Above")
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  {data.earning_trend?.length > 0 ? (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {trend.effective_hourly_rate} PKR/hr
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="font-bold text-slate-900 text-lg">
                              PKR {trend.net?.toLocaleString()}
                            </p>
                          </div>
                        </div>
                      ))}
                    ...
                    <p className="text-slate-500 text-sm">
                      No earning trend data available.
                    </p>
                  )}
                </div>
              </div>
